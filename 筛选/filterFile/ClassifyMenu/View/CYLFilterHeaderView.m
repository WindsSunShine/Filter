...
#pragma mark --------  添加的分割线
  ...
  _cureOfLine = [[UIView alloc] initWithFrame:CGRectMake(kCureOfLineOffX, CYLFilterHeaderViewHeigt-kCureOfLineHight, [UIScreen mainScreen].bounds.size.width - 2 * kCureOfLineOffX, kCureOfLineHight)];
    _cureOfLine.backgroundColor = [UIColor colorWithRed:188.0 / 255.0 green:188.0 / 255.0 blue:188.0 / 255.0 alpha:1.0];
    [self addSubview:_cureOfLine];
    ...
    _cancleBtn.frame = CGRectMake(CGRectGetMidX(self.frame)-60, 6, 120, 25);
...
-(void)cancelButtonAction{
    ...
    if ([self.delegate respondsToSelector:@selector(filterFootViewCleanClicked:)]) {
        [self.delegate filterFootViewCleanClicked:self.cancleBtn];
    }
